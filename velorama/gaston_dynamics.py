...
def plot_spatial_dag(dag_adjacency_matrix, coords_mat, isodepth=None, size=20):
    df = pd.DataFrame(dag_adjacency_matrix)

    # Find the indices where df equals 1
    rows, cols = np.where(df == 1)

    dag_adjacency_arr = []

    # Iterate over the indices and collect the (row, column) pairs
    for row, col in zip(rows, cols):
        dag_adjacency_arr.append((row, df.columns[col]))

    points = coords_mat

    plt.figure(figsize=(30, 30))

    # Check if isodepth is provided
    if isodepth is not None:
        plt.scatter(points[:, 0], points[:, 1], c=isodepth, cmap='viridis', label='Points', s=size)
        #plt.colorbar(label='Isodepth')  # Add a color bar to show the isodepth scale
    else:
        plt.scatter(points[:, 0], points[:, 1], c='blue', label='Points', s=size)

    ax = plt.gca()  # Get the current Axes instance

    for pair in dag_adjacency_arr:
        i, j = pair
        arrow = FancyArrowPatch((points[i, 0], points[i, 1]), (points[j, 0], points[j, 1]),
                                arrowstyle='->,head_width=1,head_length=2', linewidth=1, color='black')
        ax.add_patch(arrow)
...
def create_lr_targets(adata, pw_fit_dict, binning_output, q=0.7, file1 = 'receptors.txt', file2 = 'ligands.txt'):
